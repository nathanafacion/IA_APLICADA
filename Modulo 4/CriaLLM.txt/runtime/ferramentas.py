...
def _carregar_tools_py(caminho_agente: Path) -> dict:
    """Carrega tools.py do agente se existir. Retorna dict nome->funcao."""
    caminho_tools = caminho_agente / "tools.py"
    if not caminho_tools.exists():
        return {}
    import importlib.util
    spec = importlib.util.spec_from_file_location("agent_tools", caminho_tools)
    modulo = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(modulo)
    ferramentas = getattr(modulo, "FERRAMENTAS", {})
    if ferramentas:
        print(f"  [ferramentas] tools.py carregado: {', '.join(ferramentas.keys())}")
    return ferramentas


def construir_ferramentas_dos_contratos(contratos: dict, caminho_agente: Path = None) -> dict:
    """Constroi o registro de ferramentas.

    Prioridade:
    1. tools.py do agente (implementacoes reais, le o disco)
    2. LLM simulando a ferramenta (fallback quando sem tools.py)
    """
    ferramentas_reais = {}
    if caminho_agente:
        ferramentas_reais = _carregar_tools_py(Path(caminho_agente))
    ...
    for habilidade in habilidades:
        ...
        if not nome:
            continue
        if nome in ferramentas_reais:
            ferramentas[nome] = ferramentas_reais[nome]
        else:
